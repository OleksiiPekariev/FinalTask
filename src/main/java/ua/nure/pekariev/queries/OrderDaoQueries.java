...
public final class OrderDaoQueries {
	...
	public static final String INSERT_NEW_ORDER_QUERY = "INSERT INTO \"order\"(id, id_user, id_car, starts, expires, total_rent_value, days_amount, driver) "
	...
	public static final String GET_ORDER_WITHIN_DATES_QUERY = "SELECT \"order\".id, account.first_name, account.last_name, account.email, account.passport_data, account.address, account.password, account.phone,account.active, role.id AS role_id, role.role, car_name.name, car_model.model, car_class.class, car.state_number, car.year, car.rent_value_per_day, car.equipment_information,car.air_conditioner, car.navigation, \"order\".created, \"order\".confirmed, \"order\".starts, \"order\".expires, \"order\".total_rent_value,\"order\".status, \"order\".status_comment, \"order\".driver, \"order\".days_amount, \"order\".paid FROM \"order\", account, car, name_model, car_name, car_model, car_class, account_role, role  WHERE \"order\".id_car = ? AND ((? BETWEEN \"order\".starts AND \"order\".expires) OR (? BETWEEN  \"order\".starts AND \"order\".expires) OR (\"order\".starts BETWEEN ? AND ? )) AND \"order\".status <= '2'";
